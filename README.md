...
1) Analyze cryptocurrency price data

2) Understand market volatility

3) Apply mathematical functions to simulate price movements

4) Build an interactive financial dashboard
...
# Column	Description
Timestamp	Time at which the trade data was recorded
...
1. Candlestick Chart with Simulation
...
````returns = np.log(data["Close"] / data["Close"].shift(1))v````
A threshold is used to classify periods as:
